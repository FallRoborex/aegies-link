...
pub enum PacketType {
    Connection    = 0, // Handshake initiation
    PlayerPosition = 1, // Unreliable — fire and forget
    GameEvent     = 2, // Reliable — must arrive
    ChatMessages  = 3, // Reliable + ordered — must arrive in order
    Snapshot      = 4, // Server → all clients, world state
    AuthChallenge = 5, // Server → client: 16-byte nonce
    AuthResponse  = 6, // Client → server: HMAC(shared_secret, nonce)
}
...
impl Packet {
    // Build a Packet from raw bytes arriving from the wire.
    // Flags are derived from packet_type rather than read from the wire,
    // keeping the on-wire format compatible with the Python test clients.
    pub fn from_bytes(data: &[u8]) -> Option<Packet> {
        ...
        let packet_type = match data[0] {
            ...
            5 => PacketType::AuthChallenge,
            6 => PacketType::AuthResponse,
            _ => return None,
        };
        ...
        let flags = match packet_type {
            ...
            PacketType::AuthChallenge  => FLAG_UNRELIABLE,
            PacketType::AuthResponse   => FLAG_UNRELIABLE,
        };
    ...
    // Serialise a Packet to bytes for sending over the wire.
